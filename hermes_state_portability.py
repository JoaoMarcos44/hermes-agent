...
class SessionPortabilityMixin:
    ...
    def export_session(self, session_id: str, include_compacted: bool = False) -> Optional[Dict[str, Any]]:
        """Export one session. include_compacted selects display history; importable payloads leave it off
        so summarized-away turns are not resurrected as live context."""
    ...
    def export_all(self, source: str = None, include_compacted: bool = False) -> List[Dict[str, Any]]:
        """Export all sessions; display history reads per-session so compaction generations are deduped."""
