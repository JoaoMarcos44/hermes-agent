...
class SessionPortabilityMixin:
    ...
    def export_session(self, session_id: str, include_compacted: bool = False,
        ...
        ``include_inactive`` is the round-trip projection: every row in storage order with its
        ``active``/``compacted`` flags, which :meth:`import_sessions` restores as archived."""
    ...
    def adopt_session_lineage_from(self, donor_db: Any, session_id: str, *, retire_donor: bool = True) -> Dict[str, Any]:
        ...
        """
        # Every row, not just live ones: the retired donor is unrecoverable, so a turn left behind
    ...
    def _normalize_import_session(self, raw: Dict[str, Any], session_id: str, messages: list) -> Dict[str, Any]:
        ...
        for message_index, message in enumerate(messages):
            ...
            # Row state from the storage-order round-trip projection; absent flags mean live rows.
            clean_message["active"] = self._coerce_or(clean_message.get("active"), int, 1)
            clean_message["compacted"] = self._coerce_or(clean_message.get("compacted"), int, 0)
    ...
    def _import_session_row(self, conn, raw: Dict[str, Any], messages: List[Dict[str, Any]], session_id: str) -> None:
        ...
        # A row exported archived (round-trip projection) must stay archived: inserted live, a
        # compaction-archived turn would re-enter model context and a rewound turn would resurface.
        # Session counters count live rows only.
        archived = [msg for msg in sanitized_messages if msg.get("_row_id") is not None and not msg.get("active", 1)]
        if archived:
            conn.executemany(
