...
async def test_start_gateway_classifies_startup_signal_exit(
    ...
    """A startup SIGTERM is restartable (False → exit 1) unless a planned-stop marker made it intentional (True → exit 0)."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    signal_state = None
    cron_started = False
...
@pytest.mark.asyncio
async def test_start_gateway_aborted_startup_service_restart_fallback(tmp_path, monkeypatch):
    """When service restart was requested mid-startup without explicit exit_code, exit code 75 is raised."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    cron_started = False

    class ServiceRestartStartupRunner:
        def __init__(self, config):
            self.config = config
            self.adapters = {}
            self._running = False
    ...
    monkeypatch.setattr("tools.skills_sync.sync_skills", lambda quiet=True: None)
    monkeypatch.setattr("hermes_logging.setup_logging", lambda hermes_home, mode: None)
    monkeypatch.setattr("gateway.run.GatewayRunner", ServiceRestartStartupRunner)
    monkeypatch.setattr("gateway.run._start_cron_ticker", fail_if_cron_starts)
    monkeypatch.setattr("tools.mcp_tool_lifecycle.shutdown_mcp_servers", lambda: None)
