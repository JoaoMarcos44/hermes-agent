...
def test_docker_config_migrate_stamps_unversioned_config_without_rewriting_values(
    tmp_path: Path,
) -> None:
    """A missing version stamp is not a below-floor config; Docker follows migrate_config()."""
    config_path = tmp_path / "config.yaml"
