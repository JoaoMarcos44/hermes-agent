...
class TestConfigYamlRouting:
    ...
    def test_tool_search_defer_is_recognized_and_coerced(self, _isolated_hermes_home, capsys):
        """The runtime-read defer list must be registered for config set/get workflows."""
        set_config_value("tools.tool_search.defer", '["todo_list", "skill_manage"]')

        assert "not a recognized config key" not in capsys.readouterr().out
        saved = yaml.safe_load(_read_config(_isolated_hermes_home))
        assert saved["tools"]["tool_search"]["defer"] == ["todo_list", "skill_manage"]
