...
# Provider error bodies meaning image payload is corrupt/undecodable (payload-specific,
# NOT route capability rejection).
_CORRUPT_IMAGE_PHRASES = (
...
def _looks_like_corrupt_image_error(error_body: str) -> bool:
    """Return True when a provider error indicates a corrupt or undecodable image payload."""
    body = str(error_body or "").lower()
    return any(phrase in body for phrase in _CORRUPT_IMAGE_PHRASES)
