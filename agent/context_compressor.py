...
from agent.redact import redact_reentry_text, redact_sensitive_text
...
def _redact_compaction_text(text: Any) -> str:
    ...
    Shares the reentry policy (``force=True``, URL credentials) so compaction
    summaries and session-search metadata cannot drift."""
    return redact_reentry_text(text)
