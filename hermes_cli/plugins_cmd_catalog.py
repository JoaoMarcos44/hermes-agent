...
def _carry_user_files(old: Path, new: Path, local: Optional[list[str]]) -> None:
    ...
    the new tree.  ``None`` means there is no git checkout, so only regular non-Python files
    that the new tree does not already ship are carried. Path-type conflicts are owned by the new tree.
    """
    ...
    for dirpath, dirnames, filenames in os.walk(old, onerror=_walk_error):
        ...
        for name in (*filenames, *links):
            ...
            if local is None:
                ...
                if src.is_symlink() or rel.suffix == ".py" or os.path.lexists(new / rel):
                    continue
