...
def _reclone_plugin_update(target: Path, source: str, previous_revision: object) -> str:
    """Update a subdirectory install by staging a fresh clone and carrying user-owned files.

    The installed subtree has no ``.git`` (#65314), so the old tree is supplied explicitly to
    the installer's pre-publication hook. Any preservation conflict aborts before the live tree moves.
    """
