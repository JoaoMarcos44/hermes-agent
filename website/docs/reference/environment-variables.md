...
| `HERMES_DESKTOP_IGNORE_EXISTING` | Set to `1` to make Desktop ignore existing local runtimes (including the managed runtime and `hermes` on `PATH`) during backend resolution. Equivalent to `hermes desktop --ignore-existing`. Explicit `HERMES_DESKTOP_HERMES_ROOT`, development-source, and `HERMES_DESKTOP_HERMES` deployment overrides still take precedence. |
